...
# Flow rate mapping based on amount in ml
FLOW_RATES = {
    5: 1.15,
    10: 1.13,
    15: 1.12,
...
def get_flow_rate(amount):
    # Find the closest matching amount in the FLOW_RATES dictionary
    closest_amount = min(FLOW_RATES.keys(), key=lambda x: abs(x - amount))
    return FLOW_RATES[closest_amount]
...
def control_pumps():
    ...
    for slot, amount in data.items():
        try:
            amount = float(amount)
            flow_rate = get_flow_rate(amount)
